...
bool ICTERM::default_valence() const
{
    return val == SHORT_EMPTY_;
}

bool ICTERM::is_charge() const
{
    return ick_iso == CHARGE_CLASS;
}

bool ICTERM::no_isotope() const
{
    return ( ick_iso == NOISOTOPE_CLASS ||
             ick_iso == CHARGE_CLASS ||
             ick_iso == "v" );
}

std::string ICTERM::to_string() const
{
...
std::ostream &operator<<(std::ostream &os, const ICTERM &term_data)
{
    os << term_data.ick;
    if( !term_data.no_isotope() ) {
        os << '/' << term_data.ick_iso << '/';
    }
    os << ":" << (  term_data.default_valence() ? "*" : std::to_string(term_data.val) )
       << ":" << std::setprecision(15) << term_data.stoich;
    return os;
}
